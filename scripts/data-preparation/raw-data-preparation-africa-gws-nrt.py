...
def create_storm_database(data_t, lats_crop, lons_crop,
                          context_lat_min, context_lat_max,
                          context_lon_min, context_lon_max):
    tir_t = np.asarray(data_t["cores"][:])
    temp_t = tir_t < 0
    ...
    dict_storm_size = {int(lab): float(np.sum(labeled_array == lab) * 9.0) for lab in core_labels}

    dict_storm_extent = {}
    for lab in core_labels:
        mask = labeled_array == lab

        lat_vals = np.asarray(lats_crop)[mask]
        lon_vals = np.asarray(lons_crop)[mask]
    ...
    for lat, lon in zip(Pmax_lat, Pmax_lon):
        ...
        lab = int(labeled_array[y_idx, x_idx])
        if lab == 0:
            continue
        ...
        storm_database[lab] = {
            ...
            "size": dict_storm_size.get(lab, 0.0),
            "mask": 1,
        }
...
for file_t in all_files:
    ...
    try:
        with Dataset(file_t0, "r") as data_t0:
            Pmax_lat0 = np.asarray(data_t0["Pmax_lat"][:]).ravel()
            Pmax_lon0 = np.asarray(data_t0["Pmax_lon"][:]).ravel()

            valid0 = (
                (Pmax_lon0 >= CONTEXT_LON_MIN) & (Pmax_lon0 <= CONTEXT_LON_MAX) &
                (Pmax_lat0 >= CONTEXT_LAT_MIN) & (Pmax_lat0 <= CONTEXT_LAT_MAX)
            )
