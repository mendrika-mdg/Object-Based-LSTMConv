...
OUTPUT_BASE  = f"/work/scratch-nopw2/mendrika/OB/evaluation/predictions/ncast-calibrated/t{LEAD_TIME}"
...
mean_arr = np.asarray(scaler["mean"])
scale_arr = np.asarray(scaler["scale"])

# isotonic calibration
calib_path = f"/work/scratch-nopw2/mendrika/OB/evaluation/calibration/ncast-nflics/isotonic_t{LEAD_TIME}.joblib"
print(f"Loading isotonic calibrator: {calib_path}")
iso = load(calib_path)
...
for year, month, day, hour, minute in tqdm(input_files, desc="Predicting"):
    try:
        ...
        flat[:, COLS_TO_SCALE] = (flat[:, COLS_TO_SCALE] - mean_arr) / scale_arr
        ...
        flat_mean = mean_pred.cpu().numpy().ravel()
        flat_cal = iso.predict(flat_mean)
        mean_cal = torch.tensor(flat_cal.reshape(mean_pred.shape), dtype=torch.float32)
...
print("All ensemble calibrated nowcasts completed on CPU.")
