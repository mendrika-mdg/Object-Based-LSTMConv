...
OUTPUT_BASE = f"/work/scratch-nopw2/mendrika/OB/evaluation/predictions/ncast-nflics/val/t{LEAD_TIME}"
...
def load_NFLICS_nowcast(y, mo, d, h, mi, lead):
    base = f"/gws/ssde/j25b/swift/nflics_nowcasts/{y}/{mo}/{d}/{h}{mi}"
    file = f"{base}/Nowcast_{y}{mo}{d}{h}{mi}_000.nc"
    ds = Dataset(file, mode="r")
    return ds["Probability"][lead, :, :]

NFLICS_ymin, NFLICS_ymax = 9, 526
NFLICS_xmin, NFLICS_xmax = 100, 597
...
for year, month, day, hour, minute in tqdm(input_files, desc="Predicting"):
    try:
        ...
        nf = load_NFLICS_nowcast(year, month, day, hour, minute, int(LEAD_TIME))
        nf = np.asarray(nf, dtype=float) / 100.0
        nf[nf <= 0] = 0.0
        nf[np.isnan(nf)] = 0.0
        nf = np.clip(nf, 0.0, 1.0)

        nf_crop = nf[NFLICS_ymin:NFLICS_ymax+1, NFLICS_xmin:NFLICS_xmax+1]

        scale_y = 512 / nf_crop.shape[0]
        scale_x = 512 / nf_crop.shape[1]
        nf_512 = zoom(nf_crop, (scale_y, scale_x), order=1)
