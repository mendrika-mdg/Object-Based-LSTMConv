...
# max speed in one hour
V_MAX = 100.0
R_EXCL = RADIUS + V_MAX * (LEAD_TIME / 60.0)
...
print("Chosen threshold (Youden):", p_thr, flush=True)
print("TPR at Youden:", float(tpr[best_idx]), flush=True)
print("FPR at Youden:", float(fpr[best_idx]), flush=True)
...
print("Hits:", int(hit), flush=True)
print("Misses:", int(miss), flush=True)
print("False alarms:", int(false_alarm), flush=True)
print("Correct negatives:", int(correct_negative), flush=True)
print("POD:", float(pod), flush=True)
print("FAR:", float(far), flush=True)


out_dir = Path("/home/users/mendrika/Object-Based-LSTMConv/outputs/initiation/radius_corrected")
out_dir.mkdir(parents=True, exist_ok=True)

out_npz = out_dir / f"ci_contingency_youden_R{RADIUS}_t{LEAD_TIME:03d}min.npz"
...
df = pd.DataFrame(
    {
        ...
        "correct_negatives": [int(correct_negative)],
        "POD": [float(pod)],
        "FAR": [float(far)],
...
out_csv = out_dir / f"ci_contingency_youden_R{RADIUS}_t{LEAD_TIME:03d}min.csv"
df.to_csv(out_csv, index=False)

out = out_dir / f"roc_ci_{RADIUS}_t{LEAD_TIME:03d}min.npz"
np.savez(
    out,
    fpr=fpr,
    tpr=tpr,
