...
output_dir = f"/work/scratch-nopw2/mendrika/OB/evaluation/ncast-nflics-full-corrected/bss-mcs-15"
...
print(f"Found {len(filtered)} files at hour={target_hour} UTC before core filtering")
...
for file_path in tqdm(filtered, desc="Accumulating pixelwise BSS"):
    ...
    gt_raw_bin = (gt > 0).astype(np.float32)
...
print(f"Used {used_scenes} MCS scenes (≥15 cores)")
...
print(f"Saved pixelwise BSS map (GT max-filtered, MCS-only) to {out_file}")
