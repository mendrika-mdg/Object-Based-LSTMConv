...
class HybridLoss(nn.Module):
    def __init__(self, window=9, pos_weight=25.0, alpha=0.7):
        super().__init__()
        self.pool = nn.AvgPool2d(window, 1, window // 2)
...
class OB2MapModel(pl.LightningModule):
    def __init__(self, lstm_hidden=256, lstm_layers=1, dropout=0.15,
        ...
        self.criterion = HybridLoss(window=9, pos_weight=pos_weight, alpha=alpha)
...
def main():
    ...
    trainer = pl.Trainer(
        ...
        strategy="ddp",
        precision="16-mixed",
        logger=WandbLogger(project="western_sahel_latent32",
