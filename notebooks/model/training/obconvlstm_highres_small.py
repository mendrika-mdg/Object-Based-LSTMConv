...
# compute FSS(window=3)
def compute_fss_window3(preds, targets):
    pool = nn.AvgPool2d(3, 1, 1)
    p = pool(preds)
    t = pool(targets)

    mse = torch.mean((p - t) ** 2)
    ref = torch.mean(p**2) + torch.mean(t**2) + 1e-8

    return mse / ref


# BCE + FSS-3
class BCEFSSLoss(nn.Module):
    def __init__(self, pos_weight=25.0, fss_weight=0.2):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        fss_loss = compute_fss_window3(probs, targets)

        return bce_loss + self.fss_weight * fss_loss
...
class OB2MapModel(pl.LightningModule):
    def __init__(self, lstm_hidden=256, lstm_layers=1, dropout=0.15,
        ...
        self.criterion = BCEFSSLoss(pos_weight=pos_weight, fss_weight=0.2)
...
def main():
    ...
    ckpt = f"/gws/nopw/j04/wiser_ewsa/mrakotomanga/OB/checkpoints/lstm_core2map_hybrid/latent32_fss3/t{lead}/seed{seed}"
    ...
    trainer = pl.Trainer(
        ...
        strategy="ddp",
        precision="16-mixed",
        logger=WandbLogger(project="western_sahel_latent32_fss3",
