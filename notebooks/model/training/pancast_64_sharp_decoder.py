...
class SpatiallyEnhancedLoss(nn.Module):
    def __init__(self, window_size=15, pos_weight=25.0, alpha=0.3, l1_prob=0.0):
        super().__init__()
        self.pool = nn.AvgPool2d(window_size, 1, window_size // 2)
        self.alpha = alpha
        self.l1_prob = l1_prob
    ...
    def forward(self, logits, targets):
        ...
        loss = self.alpha * bce + (1 - self.alpha) * fss
        if self.l1_prob > 0:
            loss = loss + self.l1_prob * probs.mean()
        return loss
...
class UpsampleConvBlock(nn.Module):
    def __init__(self, c_in, c_out, dropout_p=0.2):
        super().__init__()
...
class SharpenedDecoder(nn.Module):
    def __init__(self, embed_dim, out_hw=(512, 512), dropout_p=0.2):
        ...
        for c1, c2 in zip(ch[:-1], ch[1:]):
            blocks.append(UpsampleConvBlock(c1, c2, dropout_p=dropout_p))
...
def main():
    ...
    ckpt = f"/gws/nopw/j04/wiser_ewsa/mrakotomanga/pancast_64_sharpened/checkpoints/t{lead:03d}min/seed{seed}"
    os.makedirs(ckpt, exist_ok=True)

    train_dl = DataLoader(
        ShardDataset(train_dir, split_by_rank=True, split_by_worker=True),
        batch_size=4,
        num_workers=4,
    ...
    trainer = pl.Trainer(
        ...
        precision="16-mixed",
        logger=WandbLogger(
            project="dev-pancast-64-sharpened",
