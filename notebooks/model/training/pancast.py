...
class Core2MapModel(pl.LightningModule):
    def __init__(self, embed_dim=128, num_heads=4, num_layers=4,
        ...
        self.criterion = SpatiallyEnhancedLoss(window_size=15,
...
def main():
    ...
    ckpt = f"/gws/nopw/j04/wiser_ewsa/mrakotomanga/pancast/final/checkpoints/t{lead:03d}min/seed{seed}/lr{lr}"
    ...
    trainer = pl.Trainer(
        ...
        logger=WandbLogger(
            project="dev-pancast",
            name=f"t{lead}_seed{seed}_lr{lr}"
        ),
        log_every_n_steps=5,
